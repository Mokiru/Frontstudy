...
渲染时间点 是 网络线程 通信 获取到HTML过后，在消息队列中添加一个渲染任务，然后渲染主线程执行渲染任务。

渲染过程可以分为8个步骤：首先html文件获取到，然后就要进行**解析**HTML，得到DOM树和CSSOM树。然后进行**样式计算**，得到最终样式表。然后进行**布局**得到布局表，对其**分层**接着**绘制**步骤将每个层生成对应的指令集，后面的步骤由合成线程进行，合成线程进行**分块**，即将每个层都分层若干块。选择离视口近的块优先**光栅化**，最后是**画**步骤，由于渲染主线程和合成线程都在渲染进程中，渲染进程不能直接进行系统调用，需要GPU进行中转，然后GPU调用系统硬件根据指引信息进行显示。
...
为了避免连续的多次操作导致布局树反复计算，浏览器会合并这些操作，当 JS 代码全部完成后再进行统一计算。所以，改动属性造成的 reflow 是异步完成的。执行JS代码会给消息队列添加一个计算任务。
